     ...
     bind:this={dialog}>
        <div class="w-full h-full relative text-center  flex flex-col items-center py-2 gap-2">
            {#each currentMod.DownloadLinks.reverse() as download, i}
                <a href={download} class="block h-fit bg-purple-600 w-fit px-7 text-white font-bold text-xl rounded-xl hover:bg-purple-500 active:scale-90 transition-all hover:shadow-2xl shadow-black shadow- py-2">{currentMod.Versions[currentMod.Versions.length - 1 - i]}</a>
            {/each}
